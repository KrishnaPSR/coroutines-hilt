...
class SweetsRepoImpl(
    ...
    override suspend fun downloadAndSaveAllSweets() {
        saveSweets(sweetsService.getAllSweets())
       // return sweetsService.getAllSweets().flatMapCompletable { saveSweets(it) }
    }

    override suspend fun downloadAndSaveNewSweets() {
        saveSweets(sweetsService.getNewSweets(sharedPrefs.sweetsUpdatedDate))
        /*return sweetsService.getNewSweets(sharedPrefs.sweetsUpdatedDate)
            .flatMapCompletable { saveSweets(it) }*/
    }

    override suspend fun newSweet(sweet: Sweet) {
         sweetsDb.addSweet(SweetDb(sweet))
    }
